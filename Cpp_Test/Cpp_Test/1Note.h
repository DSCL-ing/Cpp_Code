 ...
 * 
 * C++只有相近类型才能发生隐式类型转换,如int double这些才可以.主要看编译器
 * 
 * ->和.不一样.后者是对象直接访问成员, ->是编译器特殊处理后才能访问成员.(指针没有明确对象范围,需要特殊处理)
 * 
 */
 ...
 * 1.const修饰的成员
 * 2.引用
 * 3.没有不带参数(写了带参数的,但没写不带参数的)的构造函数的自定义类型成员(类)
 ...
 * 2.静态成员不能给缺省参数(缺省参数是给构造函数的初始化列表使用的,静态不走构造)
 * 
 * 场景:
 ...
 * --->const引用可以延长生命周期,将匿名对象的生命周期和xx的生命周期同等 
...
//泛型编程 -- 模板 --编译器自动识别类型编译生成相应类型的函数
/**
 * 关键字tamplate 
 * 语法:
 * 1.一个模板类型
 * template<class T>  -- T可以取任意值,一般用大写
 * void Swap(T& x, T& y)
 * {
 *	 T tmp = x;
 *	 x = y;
