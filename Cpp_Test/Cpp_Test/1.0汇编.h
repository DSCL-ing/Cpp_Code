...
/*
在栈上申请的变量是在 运行时 开辟的空间. 栈空间的变量是以基地址+偏移量的方式写在代码里的.如ebp+4,或esp-4,栈顶,栈底+偏移量...
EBP (extended base pointer)  扩展基址指针寄存器  //栈底固定不变 高地址(地址>=栈顶)
ESP（Extended Stack Pointer）为扩展栈指针寄存器，是指针寄存器的一种，用于存放函数栈顶指针 //栈顶会改变,栈大小增加,栈向下增长,栈顶往地址减小的方向移动
*/ 
