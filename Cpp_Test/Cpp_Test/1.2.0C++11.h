...
非静态成员函数的缺省值
模板函数的缺省值
    该特性使 调用模板函数时可以不写模板参数,不写模板参数就能够自动推导模板参数类型. 当无法推导出类型时,就会使用缺省值(没有缺省值就会报错).

*/

//模板右尖括号优化
/*
在C++98前 vector<vector<int>> v;这样的语法可能无法通过编译.因为>>可能会被识别成流提取操作符,导致编译失败
    需要写成vector<vector<int> > 这样才可以正确通过编译.
在C++11后，这样的语法已经被优化,能够正确通过编译
