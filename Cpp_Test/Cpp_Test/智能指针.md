...
不增加引用计数,仅作为资源的观察者;如果需要管理资源需要借助shared_ptr,因此提供了一个lock()函数来构造一个临时的shared_ptr,通过s_ptr来观察资源,如果s_ptr为nullptr,说明资源为空,否则说明资源存在

> 我们解决循环引用问题仅需这两个作用(不增加引用计数,观察资源)即可,因此有了weak_ptr
...
{
    int _data;
    std::weak_ptr<ListNode> _prev;							//改成weak_ptr即可解决
...
int main(){
    ...
    auto bar = std::make_shared<int>(10);		//都存了一个整型10
