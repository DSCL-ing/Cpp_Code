 ...
 * 
 * 
 * 对于字符串常量和string , 可以相互隐式类型转换(字符串常量可以和string强制转换,走构造函数),可以不用实现多个重载 --
 * 实现多个重载目的是追求极致效率
 * 
 * \0在[_size]处 --记得要加\0 ,\0不算有效字符,不用++size  
 * 不加\0的话,字符串会停不下来(囤囤囤囤囤囤...下去)
 * 
 * 历史原因,string有100多个接口,那时stl还没有,,早期遗留问题
 * 
 */
 ...
 * 内存问题报错 --> 提示析构 
 * 
 * VS下的sizeof(string) = 28,会额外多出一些空间 --- 空间换时间
 * VS在string内还有一个buff数组,当字符串比较小时(15+1),字符串存放在buff内.字符串大于16后,就存放在_str了
 * g++的string在32位下是4字节， 64位下是8字节 ， 采用 ‘写时拷贝’（写的时候再拷贝,谁写谁拷贝,赌博,赌它不使用,不写我就赚了） ，意思是就只有一个指针
 * 
 * .
 */
 ...
 * 8.shrink_to_fit
 * 功能:缩容至合适
 * 注释:很多编译器不支持原地缩容,几乎都是异地,缩容代价很大,很少会缩容
 * 
 * 
 * 
 * .
 ...
 * 
 * 范围for底层是迭代器,原理类似宏,傻瓜式替换,语法糖,甜,但傻白甜,迭代器名字必须和规定一样,不然就无法识别
 * 
 */
//迭代器
/**
