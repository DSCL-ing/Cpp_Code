 ...
 *
 * $. 左值不是只能出现在等号左边,也可以出现在等号右边 --- 只要是能取地址的值
 *
 *
 * 2.什么是右值?
 * 右值：不能进行取地址的称为右值。右值包括：临时对象、匿名对象、临时变量、匿名变量、字面值常量
 *
 */
 ...
 */
  ...
  *  -- 补充:move符合以下两点之一才能资源转移1.能生成默认的移动构造 2.支持了移动构造
  *
  * $ move有时是把两个资源直接交换 -- 资源交换
  *
  */
   ...
   */
   ...
   int main()
   {
    ...
    void test2()
    {
        test2::test::list<test::string> lt;    //建一个string链表       -- empty_init() -- 空结点也是将匿名对象移动构造
        string s1("hello world"); //建一个string对象-- 带参构造
        lt.push_back(s1);               //                       -- 深拷贝 - 测试;
        lt.push_back(string("hello world")); //匿名对象    -- 移动构造
