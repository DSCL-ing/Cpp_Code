 ...
 * $ 位运算取块用除.取位用模 --- 模从0开始,可以这种原因计算机很多以0开始
 ...
 字节序/比特序
 字节序:字节与数字位存放的位置关系,小端是低位对低位,大端是低比特位对数据高位
 比特序:低位对应地址低位或低位对应地址高位;一般物理机器随字节序方案;但是调试通常以人类可读方式显示,即大端序

 左移右移操作
 左移:低位比特向高位比特移动;以人类视角看待低位在右,高位在左,即左移,因此得名左移;
    左移运算:补零还是补1,记住左移是乘以2 ===> 因此补零
 右移:同理
    右移运算:右移是除以2,补0 (负数符号位保持不动,左边加个0,因为总不能除以2就把符号去掉了);补码的话可以先处理源码后再转换过去理解(辅助记忆);

位图占大小
1G = 10亿字节 = 2.5亿个数据
4G = 40亿字节 = 10亿个整型

100亿整型(字节) = 100亿*4字节 =  40G ;
100亿整型(比特) = 100亿*1比特 =  40G/32 = 1.25G
...
运用:
快速确定不存在(准确)
存在时再去查找数据库(减少了访问数据库的次数),或者查找其他数据源
